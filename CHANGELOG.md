...
- Expanded Checks now groups Success, Skipped, and Neutral outcomes under a nested, selectable **Valid Results** disclosure that starts collapsed; Failure/Error and Pending outcomes remain directly visible. TUI navigation, connectors, incremental-search ancestry, `rollup report`, and the web dashboard all preserve the two-level hierarchy, including per-PR fold state for both levels.
...
- Authored Checks now starts expanded when any required or optional check has failed/errored and collapsed when failure-free. Explicit TUI and tab-local web fold choices override that data-driven default across refreshes and check-state changes; `h` on a valid check returns to Valid Results, while `h` on an actionable check returns to Checks.
