...
### Added
- TUI-only `c` keybinding (Me view) that copies an "address this …" agent prompt to the system clipboard. On a single open comment it copies `Please address {url} in {branch}. Use a worktree …` using the comment's permalink and the owning PR's source branch; on the "Open comments" header it copies the same instruction with a bulleted list of every unresolved comment URL under that PR. The same key also works on the **Checks** subtree: on a single check it copies `Please address the check {name} ({url}) in {branch}. Use a worktree …` (the check's details URL, falling back to the PR URL, so it works on passing checks under Valid Results too); on the "Checks" header it copies one prompt bulleting every *failing* check (`Failure`/`Error`) under that PR, and is a no-op when none are failing. When the PR's source branch is empty the `" in {branch}"` phrase is dropped entirely, the footer confirms the copy (or reports a clipboard error), and the binding is a no-op on any other row. `rollup report` and the web UI are unchanged.
