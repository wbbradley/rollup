...
- Authored PR rows with rendered children are collapsible in both the TUI and web dashboard. Folding a PR keeps its label visible while hiding its complete Checks, Reviewers, Open comments, and Stacked PRs subtree; reopening restores the independent inner section folds. TUI fold choices survive data refreshes (with temporary state for filtered search), browser choices remain tab-local through page reloads, and `rollup report` stays fully expanded.
- TUI-only `p` keybinding (Me view) that copies a terse review-request block for the selected Authored subtree to the system clipboard: one `{url} - {title}` line per PR — the title's leading conventional-commit prefix (`feat`, `fix`, `chore`, …, with optional `(scope)`/`!`, matched case-insensitively) stripped, and ` - DRAFT` appended for draft PRs — joined by newlines. Scope follows `c`'s subtree semantics (a PR row covers its whole stack, a **Stacked PRs** header only its descendants, a **repo header** every PR in the repo) but gathers *all* PRs regardless of comments/checks, and a non-container row copies just the owning PR's line. `rollup report` and the web UI are unchanged.
- TUI-only `c` keybinding (Me view) that copies **one** aggregate "please address the following" agent prompt for the selected Authored node's subtree to the system clipboard. It gathers every unresolved comment and failing check (`Failure`/`Error`) in that subtree, grouped per PR (`In {branch} (#{number} {title}):` sub-headers with bare-permalink comment bullets and `check {name} ({url})` check bullets, falling back to the PR URL): a PR row includes its whole stack, a **Stacked PRs** header only its descendants, a **repo header** every PR in the repo, the **Open comments**/**Checks** headers that one PR's comments/failing checks, and a single comment or single check just that item (a single check works even when it is passing). The trailer is branch-count aware — one worktree for a single branch, or a worktree and optional sub-agent per branch across several — and empty or prompt-notion-free subtrees (Reviewers, a reviewer, Valid Results) report `c: nothing to address here`. This supersedes the earlier single/parent comment-copy and check-copy behaviors, unifying them into one format. `rollup report` and the web UI are unchanged.
- Authored-pane repo grouping headers are now landable and collapsible: `Enter` opens the repository, and `l`/`h` (Right/Left) expand or collapse the whole repo, hiding or showing all of its PR subtrees. `rollup report` stays fully expanded.
- Expanded Checks now groups Pending outcomes under a nested, selectable **Pending** disclosure and Success, Skipped, and Neutral outcomes under a separate **Valid Results** disclosure; both start collapsed, while Failure/Error outcomes remain directly visible. TUI navigation, connectors, incremental-search ancestry, `rollup report`, and the web dashboard preserve the hierarchy with independent per-PR fold state.
...
### Changed
- The TUI `c` action now lists review-comment API IDs with 100-character body excerpts and a reusable `gh api` command containing a literal `$comment_id` placeholder instead of emitting browser permalinks.
- Every Authored PR's section subtree is indented one additional tree level beneath its PR row in the TUI, `rollup report`, and web dashboard, including recursively stacked PRs and filtered search results.
- The animated `Loading…` refresh indicator now appears right-aligned in the upper pane's title bar instead of occupying the footer; footer errors, action status, and loaded-time details remain unchanged.
- Authored Checks now starts expanded when any required or optional check has failed/errored and collapsed when failure-free. Explicit TUI and tab-local web fold choices override that data-driven default across refreshes and check-state changes; `h` on a pending check returns to Pending, `h` on a valid check returns to Valid Results, and `h` on a failing/errored check returns to Checks.
...
### Removed
- The **People** pivot view and its old `p` keybinding, removed from both the TUI and `rollup report`. `rollup report` now prints four sections (Reviewing, Releases, Authored, Recently merged) with no "People" section, and `p` is now the Authored review-request copy described above. The Recently-merged pane is scoped to the viewer and the Reviewing authors in every remaining view (unchanged from what those views already showed).
