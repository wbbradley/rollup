...
### Added
- A loopback-only, read-only web companion for interactive `rollup`: `http://127.0.0.1:7011/` mirrors the Authored-by-me repository/stack hierarchy with clickable checks, reviewers, comments, and PRs, while `/merged` shows the Me view's recently merged set. It shares the TUI's latest successful fetch, reports loading/warning/error state, preserves last-good data after refresh failures, and exits cleanly if the port is unavailable. `rollup report` does not start the server.
- Per-PR **Checks** section in the `Authored by me` pane: a collapsible node (first under each PR, collapsed by default) whose header shows an at-a-glance merge-readiness signal computed from **branch-protection-required checks only** — `✓` green (all required pass, or none required), `✗` red (a required check failed), `◉` pending, `○` unknown — plus a `passed/total required` ratio. A failing *non-required* check never turns the signal red; expanding lists every check with non-required ones dimmed and tagged `(not required)`. `Enter` on a check opens its details URL (falling back to the PR); `h`/`l` collapse/expand like the other sections. Required flags come from one extra batched GraphQL call per refresh; PRs with no checks omit the section. Applies to both the TUI and `rollup report`.
