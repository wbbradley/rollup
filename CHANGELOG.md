...
### Added
- TUI-only `c` keybinding (Me view) that copies an "address this comment" agent prompt to the system clipboard. On a single open comment it copies `Please address {url} in {branch}. Use a worktree …` using the comment's permalink and the owning PR's source branch; on the "Open comments" header it copies the same instruction with a bulleted list of every unresolved comment URL under that PR. When the PR's source branch is empty the `" in {branch}"` phrase is dropped entirely, the footer confirms the copy (or reports a clipboard error), and the binding is a no-op on any other row. `rollup report` and the web UI are unchanged.
